...
terraform {
  backend "s3" {
    bucket                  = ""
    key                     = "recipes-production/terraform.tfstate"
    region                  = "ap-northeast-1"
    shared_credentials_file = "./.aws"
  }
}
...
module "autoscaling_web" {
  ...
  ecs_service_name = "${var.name}_web"
...
module "metric_alarms" {
  ...
  name                                = var.name
  action                              = module.notify_slack.this_slack_topic_arn
  db_instance_id                      = module.db.id
  elasticache_member_clusters         = module.sidekiq_redis.elasticache_member_clusters
...
module "rds-error-log" {
  source    = "../modules/cloudwatch-exporter"
  name      = "${var.name}_rds_error_log"
  log_group = "/aws/rds/instance/${local.fmt_rds_name}/error"
  s3_prefix = "rds_error_log"
  s3_bucket = local.cloudwatch_log_groups
  schedule  = "cron(0 19 ? * 3 *)"
}

module "rds-general-log" {
  source    = "../modules/cloudwatch-exporter"
  name      = "${var.name}_rds_general_log"
  log_group = "/aws/rds/instance/${local.fmt_rds_name}/general"
  s3_prefix = "rds_general_log"
  s3_bucket = local.cloudwatch_log_groups
  schedule  = "cron(0 19 ? * 3 *)"
}


module "rds-slowquery-log" {
  source    = "../modules/cloudwatch-exporter"
  name      = "${var.name}_rds_slowquery_log"
  log_group = "/aws/rds/instance/${local.fmt_rds_name}/slowquery"
  s3_prefix = "rds_slowquery_log"
  s3_bucket = local.cloudwatch_log_groups
  schedule  = "cron(0 19 ? * 3 *)"
}
